...
public class UpdateChecker {
    ...
    void check() {
        ...
        try {
            ...
            JsonArray dataJSONArray = GSON.fromJson(httpClient.send(
                    HttpRequest.newBuilder(new URI("https://api.modrinth.com/v2/project/enclosure/version")).build(),
                    HttpResponse.BodyHandlers.ofString()
            ).body(), JsonArray.class);

            List<JsonElement> dataList = new ArrayList<>(dataJSONArray.size());

            for(JsonElement ele : dataJSONArray){
                dataList.add(ele);
            }

            if(dataList.size() > 0){
                Optional<JsonObject> latest = dataList.stream()
